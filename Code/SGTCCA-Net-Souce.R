                       ...
                       min_mod_size = 20, max_mod_size = 300, network_preference = 'small', saving_dir = NULL)
{
  ...
  for (j in 1:num_solutions)
  {
    ...
    Network_Summarization_PPR_single(Abar = abar[[j]],CorrMatrix = cor_mat, type = types, data = combined_data, Pheno = pheno,
                                   ModuleIdx = i, min_mod_size = min_mod_size, max_mod_size = max_mod_size, method = summary_method, network_preference =  network_preference, saving_dir = saving_dir)
  }
  
  cat('SGTCCA-Net running complete!')
